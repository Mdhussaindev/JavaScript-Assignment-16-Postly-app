...
signupBtn.addEventListener("click", ()=>{
  ...
  if(!name || !password){Swal.fire({icon:"warning",title:"All fields required"});return}
...
loginBtn.addEventListener("click", ()=>{
  ...
  if(!validUser){Swal.fire({icon:"error",title:"Invalid credentials"});return}
...
function showPostsPage(){
  ...
  if(!user) return
  loginForm.classList.remove("active");signupForm.classList.remove("active");postsPage.classList.add("active")
...
function renderPosts(){
  ...
  if(userPosts.length===0){postsList.innerHTML="<p>No posts yet.</p>";return}
  userPosts.forEach(post=>{
    let div=document.createElement("div");div.classList.add("post-card")
    div.innerHTML=`<h4>${post.user}</h4><p>${post.content}</p><button class="editBtn">Edit</button><button class="deleteBtn">Delete</button>`
    let editBtn = div.querySelector(".editBtn"), deleteBtn = div.querySelector(".deleteBtn")

    editBtn.addEventListener("click", ()=>{
      Swal.fire({
        title:"Edit Post",
        html:`<input id="swalTitle" class="swal2-input" placeholder="Title" value="${post.user}"><textarea id="swalContent" class="swal2-textarea" placeholder="Content">${post.content}</textarea>`,
        confirmButtonText:"Update",
        preConfirm:()=>{let newTitle=document.getElementById("swalTitle").value,newContent=document.getElementById("swalContent").value;if(!newTitle||!newContent){Swal.showValidationMessage("Both fields required")}return{newTitle,newContent}}
      }).then(result=>{post.user=result.value.newTitle;post.content=result.value.newContent;localStorage.setItem("posts",JSON.stringify(posts));renderPosts();Swal.fire({icon:"success",title:"Updated",timer:1000,showConfirmButton:false})})
    })

    deleteBtn.addEventListener("click", ()=>{
      Swal.fire({title:"Delete this post?",icon:"warning",showCancelButton:true,confirmButtonText:"Yes"}).then(res=>{if(res.isConfirmed){posts=posts.filter(p=>p.id!==post.id);localStorage.setItem("posts",JSON.stringify(posts));renderPosts();Swal.fire({icon:"success",title:"Deleted",timer:1000,showConfirmButton:false})}})
    })
